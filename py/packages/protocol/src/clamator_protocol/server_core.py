...
class RpcServerCore:
    ...
    def register_service(self, contract: Contract, instance: Any) -> None:
        ...
        self._services[contract.service] = _ServiceEntry(
            contract=contract, handler_instance=instance
        )

    def _dispatcher(self, service_name: str) -> Dispatcher:
        async def dispatch(env: Envelope) -> dict[str, Any] | None:  # noqa: PLR0911,PLR0912  # dispatcher has many error-response branches
            ...
            if entry is None:
                return None if is_notification else build_error_response(
            ...
            try:
                ...
                return build_error_response(
                    rpc_id, -32602, "Invalid params", {"errors": e.errors()}
                )
            ...
            try:
                ...
                if is_notification:
                    return None
                return build_error_response(
                    rpc_id, -32603, "Internal error", exception_to_error_data(e)
                )
            finally:
